...
git archive --format=tar HEAD | gzip -9 > ${HOME}/rpmbuild/SOURCES/ovirt-engine-extension-aaa-jdbc-$VERSION.tar.gz
...
sed \
    ...
    > ovirt-engine-extension-aaa-jdbc.spec

rpmbuild \
    --define "_topmdir $PWD/rpmbuild" \
    --define "_rpmdir $PWD/rpmbuild" \
    -ba --nodeps ovirt-engine-extension-aaa-jdbc.spec
